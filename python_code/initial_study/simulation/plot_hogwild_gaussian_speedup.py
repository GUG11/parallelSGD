    ...
    for s in ss:
        ...
        for scheme in schemes:
            ...
            for i in xrange(len(end_it)):
                speed_up.append(end_it[0] / end_it[i] * Ps[i])
            ax.plot(Ps, speed_up, label=scheme, lw=2)

        utils.set_axis(ax, xlabel='number of cores', ylabel='speed up', xticks=None, yticks=None, xlim=None, fontsize=30)
        plt.tight_layout()
        savefile = 'speedup_n%d_d%d_T%d.svg' % (n, d, nit)
        save_dir = os.path.join(filedir, savefile)
        fig.savefig(save_dir)
        savefile = 'speedup_n%d_d%d_T%d.pdf' % (n, d, nit)
        save_dir = os.path.join(filedir, savefile)
        fig.savefig(save_dir)
        savefile = 'speedup_n%d_d%d_T%d.jpg' % (n, d, nit)
        save_dir = os.path.join(filedir, savefile)
        fig.savefig(save_dir)
        plt.cla()
