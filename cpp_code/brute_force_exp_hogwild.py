"""
Brute force enumeration on hogwild Gaussian
...
if __name__ == '__main__':
    ...
    for s in ss:
        ...
        for n in ns:
            for d in ds:
                for P in Ps:
                    filepattern = '*_n%d_d%d_T%d_ths%d_gamma*' % (n, d, nit, P)
                    ...
                    while ret != 0:
                        for scheme in schemes:
                            ...
                            while tol < hi - lo or ret != 0:
                                ...
                                mid = (lo + hi) / 2
                                cmd = ['./hogwild_gaussian', str(n), str(d), str(s), str(P), str(mid), str(nit), '100', '100', scheme]
                                print(' '.join(cmd))
                                ret = call(cmd)
                                if ret == 0:
                        ...
                        for scheme in schemes:
                            cmd = ['./hogwild_gaussian', str(n), str(d), str(s), str(P), str(gamma), str(nit), '100', '100', scheme, 'save']
                            print(' '.join(cmd))
                            time.sleep(1)
                            ret = call(cmd)
                            if ret != 0:
                                ...
                                filename = '%s_n%d_d%d_T%d_ths%d_gamma%f' % (scheme, n, d, nit, P, gamma)
